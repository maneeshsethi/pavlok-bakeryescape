...
function render(){
	...
	if(onPlayScreen || onGameOverScreen){
		...
		if(onPlayScreen){
			...
			var moduleX = Math.floor((CANVAS_WIDTH / 4) - (moduleInfo.width / 2));
			var moduleY = Math.floor(y);
			var mX = moduleX;
			var mY = moduleY;
			
			//Draw trail
			//Animation, so we choose from trail1 and trail2
			var trailChoice = Math.sin((new Date() - gameStartTime) / 1000 * 30) > 0 ? 1 : 2;
	...
	if(onGameOverScreen){
		...
		if(timeSinceDeath < 2){ //Draw flash (yes, this is overdraw; whatevs)
			var useBlack = Math.sin(timeSinceDeath * 15) > 0; //Display ~15 flashes in the time
			if(useBlack){
